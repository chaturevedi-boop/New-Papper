...
import { Smartphone, FileSpreadsheet, Database, HelpCircle } from 'lucide-react';

export type TabType = 'drops' | 'billing' | 'masters' | 'help';
...
// Fixed bottom bar for the 4 primary sections - always one tap away, the modern
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-[0_-2px_12px_rgba(0,0,0,0.06)] pb-[env(safe-area-inset-bottom)]">
      <div className="max-w-7xl mx-auto relative grid grid-cols-4 h-16">
        {/* Animated active indicator - a slim pill that slides beneath the active item */}
        <div
          className="absolute top-1.5 h-1 w-10 rounded-full bg-emerald-500 transition-transform duration-300 ease-out"
          style={{ left: `calc(${activeIndex * 25}% + 50% / 4 - 20px)` }}
        />
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-bold ${isActive ? '' : 'font-semibold'}`}>{item.label}</span>
            </button>
          );
        })}
      </div>
